...
$start = ' - ' #Change the text within the 's to whatever substring you're looking to start trimming from
$end = '.' #This preserves the file extension.

Get-ChildItem | Rename-Item -NewName { $_.Name -replace $_.Name.SubString($_.Name.IndexOf($start)),$_.Name.SubString($_.Name.LastIndexOf($end)) }
